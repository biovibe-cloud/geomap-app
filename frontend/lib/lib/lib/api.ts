 ...
 * free of React state.
 *
 * Components never call this directly — data flows through hooks (e.g.
 * useMaps) so screens receive plain props and stay easy to test/preview.
 */
...
/** Raw shape returned by `GET /maps`. */
...
export const api = {
  getMaps: (token?: string) =>
    request<MapsResponse>("/maps", token).then((r) => r.maps),
};
